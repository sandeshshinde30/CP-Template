...
long long binPow(long long a, long long b) {
    long long res = 1;
    while (b > 0) {
        if (b & 1) res *= a;  // multiply if current bit is 1
        a *= a;               // square the base
        b >>= 1;              // shift exponent
    }
    return res;
...
void topoDFSUtil(int u, vector<vector<int>> &adj, vector<bool> &visited, stack<int> &st) {
    visited[u] = true;
    for (int v : adj[u]) {
        if (!visited[v]) topoDFSUtil(v, adj, visited, st);
    }
    st.push(u);
}
...
vector<int> topoSortDFS(int n, vector<vector<int>> &adj) {
    ...
    stack<int> st;
    for (int i = 0; i < n; i++) {
        if (!visited[i]) topoDFSUtil(i, adj, visited, st);
    }
...
vector<vector<int>> kosarajuSCC(int n, vector<vector<int>> &adj) {
    ...
    function<void(int, vector<int>&)> dfs2 = [&](int u, vector<int> &scc) {
        visited[u] = true;
        scc.push_back(u);
        for (int v : adjT[u]) if (!visited[v]) dfs2(v, scc);
    };
